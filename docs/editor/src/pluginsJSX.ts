...
export const jsx = () => [javascript({ jsx: true })];

export function cssInJsx() {
  return new LanguageSupport(
    jsxLanguage.configure({
      wrap: parseMixed((ref, input) => {
        if (ref.name != "JSXElement") return null;

        const { from, to } = ref.node.firstChild!;

        if (input.read(from + 1, from + 6) != "style") return null;
...
export function autoElementSplit() {
  return keyBind({
    ...
    run(target: CommandTarget) {
      ...
      const notBetweenTags = state.selection.ranges.find(
        (range) => state.sliceDoc(range.from - 1, range.to + 1) !== "><"
      );

      if (notBetweenTags) return false;
