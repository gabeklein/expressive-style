...
export const readOnly = () => [code(), EditorView.editable.of(false)];
...
export function onKey<T extends string>(
  key: T,
  action: (key: T) => boolean | void
) {
  return keyBind({
    key,
    run: () => action(key) !== false,
  });
}
...
export function command(key: string | Record<string, Action>, action?: Action) {
  if (typeof key === "object")
    return Object.entries(key).map(([key, action]) => command(key, action));

  if (action) return onKey(`Mod-${key}`, action);
}
