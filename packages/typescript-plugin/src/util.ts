...
export function findIdentifierNodeAtPosition(
...
export function isPositionInLabelStatement(
  sourceFile: ts.SourceFile,
  position: number
): { isInLabel: boolean; identifierName: string | null } {
...
export function isExpressionInLabelStatement(
  sourceFile: ts.SourceFile,
  node: ts.Node
): boolean {
  let parent = node.parent;
  ...
  while (parent) {
    if (parent.kind === ts.SyntaxKind.LabeledStatement) return true;
...
export function labelContainsNormalControlFlow(
  ...
  function checkForControlFlow(node: ts.Node) {
    ...
    if (!containsNormalConstruct) ts.forEachChild(node, checkForControlFlow);
  }
