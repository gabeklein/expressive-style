...
for (const [a, b] of INVERSE_OP) INVERSE_OP.set(b, a);
...
export function isBinaryAssertion(
  ...
  if (t.isBinaryExpression(exp) && ASSERT_OP.has(exp.operator)) return true;
...
export function inverseExpression(exp: BinaryExpression) {
  ...
  if (inverse) return t.binaryExpression(inverse, exp.left, exp.right);
