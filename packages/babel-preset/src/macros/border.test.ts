...
it("detects args in any order", () => {
  expect(border("dashed", "red", 2)).toEqual({ border: ["red", "dashed", "2px"] });
});

it("style-only arg uses defaults for rest", () => {
  expect(border("dotted")).toEqual({ border: ["black", "dotted", "1px"] });
});

it("number-only arg is width", () => {
  expect(border(3)).toEqual({ border: ["black", "solid", "3px"] });
});
