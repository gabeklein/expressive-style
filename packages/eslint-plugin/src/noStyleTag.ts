...
export const noStyleTag: Rule.RuleModule = {
  meta: {
    ...
    messages: {
      invalidTag:
        'JSX tag name "{{tag}}" is not a valid HTML5 or SVG tag. Converting to div with {{tag}} as an attribute.',
    },
  },
  create(context: Rule.RuleContext) {
    return {
      JSXOpeningElement(node: any) {
        ...
        if (isStandard(tag) || tag === "this") return;
        ...
        context.report({
          ...
          fix(fixer) {
            ...
            const fixes = [fixer.replaceText(node.name, `div ${safeAttr}`)];

            if (!node.selfClosing && node.parent?.closingElement?.name)
              fixes.push(
                fixer.replaceText(node.parent.closingElement.name, "div")
              );
