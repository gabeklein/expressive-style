...
export const thisIsDeprecated: Rule.RuleModule = {
  meta: {
    ...
    docs: {
      description:
        "The <this> JSX element and bare this attribute are deprecated. Use fragments or explicit props forwarding instead.",
      recommended: true,
    },
    schema: [
      {
        ...
        properties: {
          ambiguous: {
            ...
            default: undefined,
            description:
              "How to fix <this>: fragment (default), div, skip, or undefined (default).",
          },
        },
    ...
    messages: {
      deprecatedThisElement:
        "<this> JSX element is deprecated. Use fragments (<>...</>) instead.",
    },
  },
  create(context: Rule.RuleContext) {
    ...
    return {
      JSXOpeningElement(node: any) {
        ...
        if (!tag || tag.type !== "JSXIdentifier" || tag.name !== "this") return;
        ...
        context.report({
          ...
          fix(fixer) {
            ...
            if (!node.attributes.length && ambiguous !== "div") {
              if (
                ambiguous === "fragment" ||
                componentHasOwnStyle(node, context)
              ) {
                ...
                if (children.length === 1 && children[0].type === "JSXElement")
                  return fixer.replaceText(
                    parent,
                    context.sourceCode.getText(children[0])
                  );
                ...
                if (node.parent?.closingElement?.name)
                  fixes.push(
                    fixer.replaceText(node.parent.closingElement.name, "")
                  );
...
function componentHasOwnStyle(node: any, context: Rule.RuleContext) {
  ...
  if (
    !functionAncestor ||
    !functionAncestor.body ||
    functionAncestor.body.type !== "BlockStatement"
  )
    return false;

  // Check for a LabeledStatement with an ExpressionStatement as its body
  return functionAncestor.body.body.some(
    (child: any) =>
      child.type === "LabeledStatement" &&
      child.body.type === "ExpressionStatement"
  );
}
